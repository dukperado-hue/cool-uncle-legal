...
const createORCAVisual = (container) => {
    ...
    const COLOR_PURPLE = "#B08A3C"

    const COLOR_REPO_MAIN = "#E0BD73"
    const COLOR_REPO = "#4a6fa8"
    const COLOR_OWNER = "#d9784f"
    const COLOR_CONTRIBUTOR = "#d96a6a"

    const COLOR_LINK = "#2E3348"
    const COLOR_TEXT = "#E9E7E1"
    ...
    function calculateLinkGradient(context, l) {
        ...
        function createGradient(l, alpha) {
            ...
            if(l.source.x !== undefined && l.target.x !== undefined) {
                ...
                // Guard against dist === 0 (can happen with very sparse datasets where nodes overlap)
                let perc = dist > 0 ? Math.min(Math.max(l.source.r / dist, 0), 1) : 0
