...
## 4d. Standee background removal — remove.bg abandoned in favor of a local script (2026-08-03)

The queued "try removebg to cut backgrounds" task (user: "ใช้ removebg ตัดฉากหลังก็ได้") turned out to be a dead end via the actual remove.bg website: its upload widget has no reachable `<input type=file>` in the accessibility tree (same class of gotcha as Gemini's own upload, see `feedback_gemini_web_automation`), and this harness's `file_upload` tool separately refuses local repo files with "only files this session is allowed to read can be uploaded" — there's no way to get an image into remove.bg's page via browser automation on this machine. No account was created, nothing was force-worked-around.

**Replacement that actually shipped: a small local Python script, no external service, no account, no rate limit.** Every turnaround sheet in this pipeline is generated on a "plain flat light-gray background" per the Gem's own system prompt (see section 3) — a near-uniform color, which is exactly the case classic flood-fill / chroma-key removal handles well without needing an ML model. Two-step approach (Pillow + numpy, both already available in this environment):

1. **Flood fill from all 4 corners** using `PIL.ImageDraw.floodfill(image, seed, (0,0,0,0), thresh=18)` — Pillow's C-implemented flood fill with a color-distance threshold, fast even at full generation resolution (~2800×1500, ~1-8s per image depending on how much of the canvas is background).
2. **Soft edge pass (vectorized numpy, not a per-pixel Python loop — that would take minutes instead of seconds)**: for any pixel the flood fill didn't zero out but that's still within `thresh * 2.2` of the sampled background color, set partial alpha proportional to color distance instead of a hard cutoff. Avoids a jagged/aliased silhouette edge.
3. **Crop to a standee**: after background removal, find the first (leftmost) contiguous blob of "opaque-ish" columns in the alpha channel — this is the sheet's front-view panel, since the 3 other views are separated from it by fully-transparent background once step 1-2 run. Tight-crop to that blob's bounding box (alpha-based, both axes) plus ~14px padding. Save to `pics/game/standees/<sprite_id>.png`.

**One real bug hit and fixed:** naive column-projection (`any column with any alpha>20 pixel = "content"`) misfired on `rabbit` — a handful of isolated 1-2px alpha specks near the canvas edge (compression/antialiasing noise, unrelated to the actual character) got mistaken for the start of the front-view panel, producing a bogus ~29×17px crop instead of the real ~625×1359px rabbit. **Fix: require a column to have at least 5 opaque pixels (not just 1) to count as "content"** before it's treated as the start/continuation of a panel — filters speckle without affecting real character silhouettes, which are always many pixels wide at any given row. Same minimum-pixel-count filter applied to the row-bbox pass within the found column range, for the same reason. Re-verified this fix doesn't regress other characters (basketball_player's box was byte-identical before/after).

**Batch results (2026-08-03):** ran across all 46 character-sheet files that had no standee yet (skipped 8 unrelated stray/unsorted raw Gemini-download files sitting in `pics/game/character-sheets/` with UUID/`Gemini_Generated_Image_*` names — not tied to any sprite id, a separate cleanup task, not touched). All 46 succeeded on the second run (post-rabbit-fix). Spot-checked 8 outputs visually across a deliberately varied sample (white-furred `rabbit`, cream-furred `dog_golden`, near-black-and-white `courtroom_cat`, dark-robed `judge`, cream-shirted `driver_female`, orange-robed `monk_young`, plus the two most re-run test cases `basketball_player`/`rabbit`) — all clean, no visible background residue, no bitten-off limbs/ears. Files land at `pics/game/standees/{armored_bodyguard,basketball_player,biker_gunman,civil_servant,civil_servant_female,comedian_uncle,court_clerk,courtroom_cat,crime_boss,defense_lawyer,delinquent,doctor,dog_golden,dog_labrador,driver,driver_female,elderly_woman,elegant_socialite,employee,employer,estate_agent,flamboyant_dandy,ghost_looking_man,hero_boy,hero_girl,hero_rival,horror_neighbor,horror_schoolgirl,husband,japanese_fighter,japanese_uncle_reviewer,judge,landlord,monk_elder,monk_young,muscular_man,plaintiff_lawyer,prosecutor,rabbit,romantic_hero,romantic_heroine,rural_uncle,samurai,saw_kad,tenant,wife}.png`.

**The script itself lives in scratch, not the repo** (same convention as `apply_contrast_fixes.py` from the contrast-fix sessions) — reconstruct it from the algorithm description above if needed again; it's short (~70 lines). **Reusable for every future character batch**: run this against any new `pics/game/character-sheets/<id>.png` with no matching `pics/game/standees/<id>.png` to backfill a standee with zero Gemini/browser-automation cost, since the Gem's background convention is stable across the whole pool.
