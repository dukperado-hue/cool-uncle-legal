...
function applyRoleFilter(role) {
  document.querySelectorAll('#sidebar [data-role]').forEach(function (el) {
    ...
    el.classList.toggle('role-hidden', role !== 'both' && r !== 'both' && r !== role);
  });
}
var ROLE_PILL_TEXT = { both: '🌐 ทุกเมนู', citizen: '👤 พลเรือน', official: '🏛️ เจ้าหน้าที่' };
var ROLE_PILL_CLASS = { both: 'role-pill-both', citizen: 'role-pill-citizen', official: 'role-pill-official' };
function roleToast(msg) {
  var old = document.getElementById('role-toast');
  if (old) old.remove();
  var d = document.createElement('div');
  d.id = 'role-toast';
  d.textContent = msg;
  d.style.cssText = 'position:fixed;top:70px;left:50%;transform:translateX(-50%);z-index:10000;background:#1d2430;color:#e8ebf0;border:1px solid #3f4a5c;border-radius:8px;padding:8px 14px;font-size:12px;box-shadow:0 4px 16px rgba(0,0,0,.4)';
...
function updateRolePill(role) {
  ...
  if (pill) {
    pill.textContent = ROLE_PILL_TEXT[role] || role;
    pill.className = '';
    pill.classList.add(ROLE_PILL_CLASS[role] || 'role-pill-both');
  }
}
(function () {
  var rs = document.getElementById('role-select');
  if (rs) {
    var saved = localStorage.getItem('bunkfai-role');
    rs.value = (saved === 'both' || saved === 'citizen' || saved === 'official') ? saved : 'both';
    applyRoleFilter(rs.value);
    updateRolePill(rs.value);
    roleToast('โหมด: ' + (ROLE_PILL_TEXT[rs.value] || rs.value));
