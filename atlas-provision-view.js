...
(function (global) {
  ...
  function injectStyle() {
    ...
    var css = [
      ...
      '.atlas-provision-view-panel{position:relative;width:min(720px,92vw);max-width:92vw;',
      ...
      '  padding:18px 32px 14px;border-bottom:1px solid var(--line,#e6e1d6);}',
      '.atlas-provision-view-breadcrumb{flex:1;min-width:0;font-size:12.5px;line-height:1.75;',
      ...
      '.atlas-provision-view-body{flex:1;overflow-y:auto;padding:26px 32px 56px;}',
      // The มาตรา number is the primary visual anchor of the reading view —
      ...
      '.atlas-provision-view-text{font-size:17px;line-height:1.9;color:var(--ink,#1f2430);}',
      '.atlas-provision-view-text p{margin:0 0 .9em;}',
      ...
      '.atlas-provision-view-para-label{display:block;font-size:11.5px;font-weight:600;',
      '  letter-spacing:.03em;color:var(--muted,#5b6472);margin-bottom:2px;}',
      ...
      // Shared section rhythm: the optional explanation / concept / case /
      // cluster sections that follow the statutory text each get one hairline
      // + one generous gap — never their own heavy card — so the drawer keeps
      // reading as a single coherent document rather than a stack of widgets.
      // Sibling top margins collapse to the larger value, so this stays one
      // consistent gap whichever of these sections is actually present.
      // Guarded by :not(:empty)/:not([hidden]) so an absent section leaves no
      // stray rule; each module still owns its own inner spacing/typography.
      '.atlas-provision-view-lectures:not(:empty),',
      '.atlas-provision-view-concepts:not(:empty),',
      '.atlas-provision-view-cases:not([hidden]),',
      '.atlas-provision-view-clusters:not(:empty){margin-top:26px;padding-top:22px;',
      '  border-top:1px solid var(--line,#e6e1d6);}',
      '.atlas-provision-view-cases{font-size:12px;}',
      ...
      '.atlas-provision-view-adjacent{display:flex;justify-content:space-between;gap:10px;',
      '  margin:32px 0 0;padding-top:22px;border-top:1px solid var(--line,#e6e1d6);}',
      '.atlas-provision-view-adj{flex:1;border:1px solid var(--line,#e6e1d6);background:var(--panel,#fff);',
      '  color:var(--ink,#1f2430);border-radius:8px;padding:10px 14px;font-size:12.5px;cursor:pointer;',
      '  font-family:inherit;font-variant-numeric:tabular-nums;text-align:center;min-height:40px;}',
      ...
      '.atlas-provision-view-full{display:inline-block;margin:16px 0 0;font-size:13px;',
      ...
      '  .atlas-provision-view-header{padding:14px 18px 10px;gap:10px;}',
      '  .atlas-provision-view-body{padding:20px 18px 44px;}',
      '  .atlas-provision-view-heading{font-size:25px;}',
      '  .atlas-provision-view-text{font-size:16px;line-height:1.85;}',
      '  .atlas-provision-view-adj{min-height:44px;}}'
    ].join('\n');
